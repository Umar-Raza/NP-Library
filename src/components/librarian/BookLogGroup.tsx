import { BookChain } from "@/lib/api/books";

export default function BookLogGroup({ group }: { group: BookChain }) {
  if (group.chain.length === 0) return null;
        ...
        {group.chain.map((entry, i) => (
          <div key={i} className="relative pl-6 pb-4 last:pb-0">
            ...
            <div className="bg-base-100 border border-base-300 rounded-field px-4 py-2.5">
              <p className="font-medium text-sm">{entry.borrowerName}</p>
              <p className="text-xs text-base-content/50">
                {new Date(entry.borrowedAt).toLocaleString("en-GB", {
                  day: "2-digit",
                  month: "short",
                  year: "numeric",
                  hour: "2-digit",
